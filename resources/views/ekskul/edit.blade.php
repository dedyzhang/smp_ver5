...
@section('container')
    {{Breadcrumbs::render('ekskul-edit',$ekskul)}}
    ...
    <div class="body-contain-customize col-12 mt-3">
        <p>Form Edit Ekskul</p>
        <form action="{{route('ekskul.update',$ekskul->uuid)}}" method="post">
            @method('put')
            @csrf
            <div class="col-12 col-sm-12 col-md-10 col-lg-10 col-xl-10 form-group">
                <label for="ekskul">Nama Ekstrakurikuler</label>
                <input type="text" name="ekskul" id="ekskul" class="form-control @error('ekskul') is-invalid @enderror" placeholder="Masukkan Nama Ekskul" value="{{old('ekskul',$ekskul->ekskul)}}">
            ...
            <div class="col-12 col-sm-12 col-md-10 col-lg-10 col-xl-10 form-group mt-2">
                ...
                <select class="form-control @error('guru') is-invalid @enderror" name="guru" id="guru" data-toggle="select" value="{{old('guru')}}">
                    ...
                    @foreach ($guru as $item)
                        <option {{$ekskul->id_guru == $item->uuid ? "selected" : ""}} value="{{$item->uuid}}">{{$item->nama}}</option>
                    @endforeach
                </select>
                <div class="invalid-feedback">Kolom ini wajib diisi</div>
            </div>
            <div class="col-12 col-sm-12 col-md-10 col-lg-10 col-xl-10 form-group mt-2">
                ...
                <select class="form-control" name="pelajaran" id="pelajaran" data-toggle="select">
                    ...
                    @foreach ($pelajaran as $item)
                        <option {{$ekskul->id_pelajaran == $item->uuid ? "selected" : ""}} value="{{$item->uuid}}">{{$item->pelajaran}}</option>
                    @endforeach
                </select>
            </div>
