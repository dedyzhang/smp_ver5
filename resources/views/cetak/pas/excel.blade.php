<table>
    <tr>
        <td style="font-size:13px" colspan="{{$ngajar->count() + 5}}" align="center"><b>Daftar Rekapulasi Nilai Sumatif Akhir Semester {{$setting->nilai ?? ""}}</b></td>
    </tr>
    <tr>
            ...
            <tr>
                ...
                @foreach ($ngajar as $item)
                    @if (isset($pas_array[$item->uuid . '.' . $siswa->uuid]))
                        <td width="5" @if ($pas_array[$item->uuid . '.' . $siswa->uuid] < $item->kkm) bgcolor="#ffd2d2" @endif align="center" style="border:1px solid #000000">
                            {{ $pas_array[$item->uuid . '.' . $siswa->uuid] }}
                        </td>
                        @php $jumlah += $pas_array[$item->uuid . '.' . $siswa->uuid]; @endphp
                    @else
