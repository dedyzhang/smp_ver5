...
@section('container')
    {{ Breadcrumbs::render('notulen-edit',$notulen) }}
    ...
    <div class="col-12 body-contain-customize mt-3">
        <p><b>Form Edit Notulen Rapat</b></p>
        <div class="form-group col-12">
            <label for="pokok_pembahasan" class="form-label">Pokok Pembahasan</label>
            <textarea class="tinymce-select validate" id="pokok_pembahasan">{!! $notulen->pokok_permasalahan !!}</textarea>
        </div>
        <div class="form-group col-12 mt-3">
            <label for="hasil_rapat" class="form-label">Hasil Rapat</label>
            <textarea class="tinymce-select validate" id="hasil_rapat">{!! $notulen->hasil_rapat !!}</textarea>
        </div>
        ...
        $('.simpan-notulen').click(function() {
            ...
            if(countError > 0) {
                ...
                var simpanNotulen = function() {
                    loading();
                    var url = "{{ route('notulen.update',':id') }}";
                    ...
                    $.ajax({
                        ...
                        success : function(data) {
                            removeLoading();
                            cAlert('green','Sukses','Notulen Rapat Berhasil Diganti',false,"{{ route('notulen.index') }}");
                        },
                        error: function(data) {
