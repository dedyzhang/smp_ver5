...
@section('container')
{{Breadcrumbs::render('absensi-kehadiran-hadir-siswa',$jenis)}}
@if ($jenis == "datang")
...
<div class="body-contain-customize col-12 mt-3 text-center">
    <p>Pastikan Kamera sudah terbuka, klik allow permission pada kamera untuk mengizinkan aplikasi untuk membuka kamera
    </p>
</div>
<script>
    var hasScan = "no";
         ...
         */
            ...
            (decodedText, decodedResult) => {
                if(hasScan == "no") {
                    loading();
                    if(decodedText !== "") {
                        var url = "{{route('absensi.kehadiran.siswa.hadir',':id')}}";
                            ...
                            success: function(data) {
                                if(data.success == false) {
                                    ...
                                    $('#audioAbsen').promise().done(function() {
                                        removeLoading();
                                        setTimeout(() => {
                                            cAlert('green',"Sukses","Sukses Menambahkan Absensi untuk User "+data.data,false,"{{route('absensi.siswa.kehadiran')}}");
                                        }, 1000);
                                    })
                                }
                            },
