...
@section('container')
    {{ Breadcrumbs::render('p3-siswa-edit',$siswa, $p3) }}
    ...
    <div class="body-contain-customize col-12 mt-3">
        <p><b>Form Edit Poin Pelanggaran, Prestasi dan Partisipasi</b></p>
        <form method="post" action="{{ route('p3.siswa.update',$p3->uuid) }}">
            @csrf
            @method('put')
            <div class="row m-0 p-0">
                <div class="m-0 p-0 mt-2 col-12 col-sm-12 col-md-10 col-lg-4 col-xl-4 form-group">
                    ...
                    <input type="date" class="form-control @error('tanggal') is-invalid @enderror" name="tanggal" id="tanggal" placeholder="Tanggal Penambahan P3" value="{{ old('tanggal',$p3->tanggal) }}" />
                    @error('tanggal')
                ...
                <div class="m-0 p-0 mt-2 col-12 col-sm-12 col-md-10 col-lg-6 col-xl-6 form-group">
                    ...
                    <select class="form-select @error('jenis') is-invalid @enderror" name="jenis" id="jenis" placeholder="Jenis Penambahan p3" >
                        ...
                        <option value="pelanggaran" @if(old('jenis',$p3->jenis) == "pelanggaran") selected @endif >Pelanggaran</option>
                        <option value="prestasi" @if(old('jenis',$p3->jenis) == "prestasi") selected @endif >Prestasi</option>
                        <option value="partisipasi" @if(old('jenis',$p3->jenis) == "partisipasi") selected @endif >Partisipasi</option>
                    </select>
                    @error('jenis')
                        <div class="invalid-feedback">Wajib Diisi</div>
                    @enderror
                ...
                <div class="m-0 p-0 mt-2 col-12 col-sm-12 col-md-12 col-lg-10 col-xl-10 form-group">
                    <label for="kategori">Kategori Yang Bisa Dipilih</label>
                    <p class="text-danger m-0 p-0" style="font-size:10px">*Dapat Dipilih Setelah Jenis P3 Dipilih, Opsional</p>
                    <select class="form-control" name="kategori" id="kategori">
                        <option value="">Pilih Salah Satu</option>
                        @foreach ($p3->kategori as $item)
                            <option value="{{ $item->uuid }}">{{ $item->deskripsi }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="m-0 p-0 mt-2 col-12 col-sm-12 col-md-12 col-lg-10 col-xl-10 form-group">
                    ...
                    <textarea class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi" id="deskripsi" placeholder="Deskripsi Poin P3">{{ old('deskripsi',$p3->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div class="invalid-feedback">Wajib Diisi</div>
                    @enderror
                </div>
