...
@section('container')
    ...
    <div class="body-contain-customize col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 mt-3">
        @if ($sudah_konfirmasi == 'belum')
            <div class="alert alert-primary" role="alert">
                ...
                <p>Nilai Olahan Belum disimpan, Berarti admin masih belum bisa menarik nilai rapor anda, silahkan check nilai anda dengan benar,lakukan perbaikan jika perlu dan jika nilai rapor sudah selesai di atur. silahkan klik tombol konfirmasi nilai dibawah. nilai yang sudah dikonfirmasi berarti sudah dikunci dan tidak bisa diubah lagi.</p>
                <hr>
        ...
        @else
            <div class="alert alert-info" role="alert">
                ...
                <p>Nilai rapor anda berhasil disimpan, Nilai untuk semester ini sudah tidak bisa diubah, terima kasih telah berusaha keras untuk mengolah nilai rapor tersebut. Minumlah segelas kopi <i class="fas fa-mug-hot"></i> untuk menghilangkan rasa stress anda, istirahatlah <i class="fas fa-bed"></i> yang cukup malam ini.</p>
                <hr>
                <p class="mb-0">Terima kasih atas perhatiannya</p>
            </div>
        @endif
    ...
    <div class="body-contain-customize col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 mt-3">
                    ...
                    @foreach ($ngajar->siswa as $siswa)
                        <tr class="siswa" data-siswa="{{$siswa->uuid}}" data-ngajar="{{$ngajar->uuid}}">
                            ...
                            <td rowspan="2" class="text-center @if ($rata2Pas < $ngajar->kkm)
                                bg-danger-subtle text-danger
                            @endif">{{$rata2Pas}}</td>
                            <td rowspan="2" class="text-center nilai-siswa ganti-nilai @if ($totalRapor < $ngajar->kkm)
                                bg-danger-subtle text-danger
                            @endif @if ($ada_temp_nilai) ada-perubahan @endif" style="cursor: pointer">{{$totalRapor}}</td>
                            <td class="fs-12 deskripsi-positif ganti-deskripsi-positif @if ($ada_temp_desk_positif) ada-perubahan @endif" data-uuid="{{$maxUUID}}" data-predikat={{$max_predikat}} style="cursor: pointer">{{$max_keterangan}}</td>
                        </tr>
                        <tr data-siswa="{{$siswa->uuid}}" data-ngajar="{{$ngajar->uuid}}">
                            <td class="fs-12 deskripsi-negatif ganti-deskripsi-negatif @if ($ada_temp_desk_negatif) ada-perubahan @endif" data-uuid="{{$minUUID}}" style="cursor: pointer">{{$min_keterangan}}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="button-place mt-3 d-grid d-sm-grid d-md-flex d-lg-flex d-xl-flex">
            @if ($sudah_konfirmasi == "belum")
                <button class="btn btn-sm btn-warning text-warning-emphasis konfirmasi-nilai"><i class="fas fa-save"></i> Konfirmasi Nilai</button>
            @else
                @can('admin')
                    <button class="btn btn-sm btn-danger hapus-konfirmasi-nilai"><i class="fas fa-trash-can"></i> Batalkan Konfirmasi Nilai</button>
                @endcan
            @endif
                    ...
                    <div class="row m-0 p-0">
                        <div class="col-12">
                            ...
                            <ul class="w-100 d-block pernyataan-konfirmasi-nilai">
                                ...
                                <li class="d-block"><span>Kelas</span> : {{$ngajar->kelas->tingkat.$ngajar->kelas->kelas}}</li>
                                <li class="d-block"><span>Semester/Tahun Pelajaran</span> : {{$semester->semester == 1 ? "Ganjil" : "Genap"}} / {{$semester->tp}}</li>
                            </ul>
                        </div>
                        <div class="col-12">
                            ...
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="persetujuan-1"> Nilai yang dikonfirmasi adalah nilai yang benar, dan sesuai dengan standar penilaian Kurikulum Merdeka</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="persetujuan-2" name="persetujuan" />
                                <label class="form-check-label" for="persetujuan-2"> Nilai yang dikonfirmasi adalah nilai yang akan dijadikan sebagai nilai rapor siswa bersangkutan</label>
                            </div>
    ...
    <script>
        @if ($sudah_konfirmasi == "belum")
            ...
            $('.hapus-nilai').click(function() {
                ...
                var idNgajar = $('#id_ngajar').val();
                var hapusNilai = () => {
                    BigLoading("Nilai sedang dihapus, mohon tidak menutup aplikasi sebelum nilai berhasil diubah");
            ...
            $('.hapus-deskripsi-positif').click(function() {
                ...
                var hapusDeskripsiPositif = () => {
                    BigLoading("Deskripsi sedang dihapus, mohon tidak menutup aplikasi sebelum nilai berhasil diubah");
                    var url = "{{route('penilaian.rapor.delete',':id')}}";
                    url = url.replace(':id',idNgajar);
                    $.ajax({
                        type: "DELETE",
            ...
            $('.setuju').click(function() {
                ...
                if(pernyataan != 2) {
                    ...
                    BigLoading('Nilai rapor sedang disimpan, mohon untuk tidak menutup aplikasi sebelum nilai berhasil disimpan');
                    var nilaiArray = [];
                    $('.siswa').each(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            setTimeout(() => {
                                removeLoadingBig();
                                cAlert("green","Berhasil",'Nilai rapor berhasil disimpan',true);
                            }, 500);
                        },
                        error: function(data) {
                            console.log(data.responseJSON.message);
            ...
            $('.hapus-konfirmasi-nilai').click(function() {
                var batalkanKonfirmasi = () => {
                    ...
                    $.ajax({
                        ...
                        headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'},
                        success: function(data) {
                            cAlert("green","Berhasil","Konfirmasi nilai rapor berhasil dibatalkan",true);
                        },
                        error: function(data) {
